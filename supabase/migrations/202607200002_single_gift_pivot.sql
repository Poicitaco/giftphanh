-- Migration: Pivot to single gift flow with star passwords

-- 1. Add recipient_password_hash and recipient_password_hint to memories
alter table public.memories
  add column recipient_password_hash text,
  add column recipient_password_hint text;
...
create function public.open_recipient_memory(p_slug text, p_session_token text, p_memory_id uuid, p_star_password text default null)
...
create function public.get_single_gift_jar()
...
set search_path = ''
as $$
  select slug, recipient_name from public.jars limit 1;
$$;
